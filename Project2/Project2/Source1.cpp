...
{	
	...
	rectangle(dst[picture_num][0], cropRect, Scalar(0, 255, 0), 3, 8, 0);
	imshow(window_name, dst[picture_num][0]);
	waitKey(27);

}
void pointsClusting()//무조건 중앙의 점을 눈동자의 점이라고 인식하는 방법
{
	int width = 320/2;
	int height = 240/2;
	int nexx, prex = width;
...
void realpointsDiff()//기준이 되는 동영상의 데이터를 가지고 눈동자 점을 찾는 방법
{
	int nexx, prex;
	int nexy, prey;
	int reali, realj= 0;
...
void pointsHitinBoundary()//내부 정사각형 안에서 눈동자 점을 찾는 방법
{
	int width = cropRect.width;
	int height = cropRect.width;
	int nexx, prex = width;
	int nexy, prey = height;
	int reali = 0;
	for (int i = 0; i < points.size(); i++)
